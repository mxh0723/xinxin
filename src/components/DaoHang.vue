        ...
        <sticky-slot class="stickyTop">
            <div class="tab">
                <ul :active="activeStep">
                    <li :class="index === 0?cor1:cor2" v-for="(item,index) in steps" :key="index" @click="jump(index)">{{item}}</li>
                </ul>
            </div>
        </sticky-slot>
...
export default {
    name: 'DaoHang',
    components:{stickySlot,Feature,Arrange,Cost,Note,Procedure,Recommend},
    data(){
        return{
